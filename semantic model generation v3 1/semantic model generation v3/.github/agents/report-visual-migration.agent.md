...
**Power BI mapping for buttons:**
- `goto-sheet` → `actionButton` visual with `action.type = "PageNavigation"` and `action.page` target
- `toggle` (show/hide zones) → `actionButton` visual with `action.type = "Bookmark"` (note: requires bookmark pairs for show/hide states — generate a comment in output noting manual bookmark setup needed)
...
> - `action.type` = `"'PageNavigation'"` for goto-sheet buttons
> - `action.page` = the `name` field from the target page's `page.json` (e.g., `"'ReportSection2'"`)
...
> **Toggle Button Notes**:
> - `action.type` = `"'Bookmark'"` — requires bookmarks to be created manually in Power BI Desktop
> - Leave `action.bookmark` empty (`"''"`) — user must assign bookmarks after opening in Desktop
...
23. Verify ALL navigation buttons from Tableau are generated as `actionButton` visuals:
    - `goto-sheet` buttons → `actionButton` with `action.type = "PageNavigation"` and valid `action.page` target
    - `toggle` buttons → `actionButton` with `action.type = "Bookmark"`
