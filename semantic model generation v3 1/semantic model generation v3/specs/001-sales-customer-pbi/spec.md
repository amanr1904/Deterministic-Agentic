...
A Power BI analyst opens the generated `.pbip` project in Power BI Desktop. The four source CSV files (Orders, Customers, Location, Products) load correctly with proper data types, and the star-schema relationships resolve without errors.
...
**Independent Test**: Open `Output/SalesCustomerDashboards/SalesCustomerDashboards.pbip` in Power BI Desktop → all four tables appear in the model view → relationships connect correctly → data preview shows rows with correct types (dates, numbers, text).

**Acceptance Scenarios**:

1. **Given** the PBIP project is opened in Power BI Desktop, **When** the model loads, **Then** all four tables (Orders, Customers, Location, Products) are visible with data rows.
...
3. **Given** the star-schema relationships are defined, **When** the model diagram is viewed, **Then** Orders connects to Customers (via Customer ID), Orders connects to Location (via Postal Code), and Orders connects to Products (via Product ID).
4. **Given** the relationships are configured, **When** cross-filtering is tested, **Then** filtering a Customer in Customers table correctly filters Orders rows.
...
### User Story 4 - Navigation and Interactivity (Priority: P3)

A report consumer navigates between the Sales Dashboard page and Customer Dashboard page using navigation buttons. A toggle button shows/hides a filter panel on each page.

**Why this priority**: Navigation enhances usability but is not required for analytical correctness. The report is functional without buttons.

**Independent Test**: Click "Sales Dashboard" button on Customer page → navigates to Sales page. Click toggle button → filter panel visibility changes.

**Acceptance Scenarios**:

1. **Given** the user is on the Customer Dashboard page, **When** they click the "Sales Dashboard" navigation button, **Then** the report navigates to the Sales Dashboard page.
2. **Given** the user is on the Sales Dashboard page, **When** they click the "Customer Dashboard" navigation button, **Then** the report navigates to the Customer Dashboard page.
3. **Given** the filter panel is hidden, **When** the user clicks the toggle button, **Then** the filter panel becomes visible.
...
- What happens when a customer has orders in CY but not PY? → `[% Diff]` measures should handle division by zero (return BLANK() when PY = 0).
- What happens when a Postal Code in Orders doesn't match any row in Location? → Unmatched rows should still appear in Orders with blank geography fields.
...
- **FR-002**: System MUST define a star-schema model with Orders as the fact table and Customers, Location, Products as dimension tables.
- **FR-003**: System MUST create relationships: Orders[Customer ID] → Customers[Customer ID] (many-to-one), Orders[Postal Code] → Location[Postal Code] (many-to-one), Orders[Product ID] → Products[Product ID] (many-to-one).
- **FR-004**: System MUST implement a "Select Year" parameter as a disconnected table with values 2020, 2021, 2022, 2023 (default: 2023) to drive all CY/PY calculations.
...
- **FR-006**: System MUST handle division-by-zero scenarios in % Diff measures by returning BLANK() when the prior year value is zero or blank.
- **FR-007**: System MUST correctly parse Order Date and Ship Date columns as date types from the CSV source.
- **FR-008**: System MUST implement navigation buttons (2 page navigation + 1 toggle per dashboard) in the report layer.
...
- **Select Year (Disconnected Parameter Table)**: Contains year values (2020–2023) for dynamic CY/PY calculation. No relationships to other tables.
...
- **SC-003**: All three relationships resolve correctly — filtering a dimension table correctly filters the Orders fact table.
